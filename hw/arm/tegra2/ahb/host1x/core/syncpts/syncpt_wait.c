...
static void handle_syncpt_update(uint32_t id, htype op, uint32_t val)
{
    struct host1x_syncpt *syncpt = &syncpts[id];
    struct host1x_syncpt_waiter *waiter, *waiter_next;

    g_assert(id < NV_HOST1X_SYNCPT_NB_PTS);
...
static void handle_base_update(uint32_t id, htype op, uint32_t val)
{
    struct host1x_syncpt_base *syncpt_base = &syncpt_bases[id];

    g_assert(id < NV_HOST1X_SYNCPT_NB_BASES);
...
/* WARNING: Assumed atomical change.  */
